...
const useMovable = (
  ...
  const start = (e: PointerEvent) => {
    ...
    const rect = resolveUnref(target)!.getBoundingClientRect()
    // ! 查找父元素的 marginLeft 和 线宽 borderWidth
    // 判断当前选择的要移动的元素有没有父元素
    const parentElement = resolveUnref(target)?.parentElement
    // let cssStyleDeclaration: CSSStyleDeclaration | undefined | null = null
    // if (parentElement) {
    //   cssStyleDeclaration = getComputedStyle(parentElement)
    // }
    const cssStyleDeclaration = parentElement ? getComputedStyle(parentElement) : null

    const _marginLeft = cssStyleDeclaration?.marginLeft
    const _borderWidth = cssStyleDeclaration?.border

    const marginLeft = _marginLeft ? parseInt(_marginLeft) : 0
    const borderWidth = _borderWidth ? parseInt(_borderWidth) : 0
    // const css = getComputedStyle(resolveUnref(target)!.parentElement!)
    // console.log(css.marginLeft)
    // // const borderW = parseInt(css.marginLeft) !== 0 ? parseInt(css.borderWidth) : parseInt(css.borderWidth)
    // const borderW = parseInt(css.borderWidth)
    // console.log(borderW)
    const pos = {
      // x: e.clientX - rect.left + borderW,
      // y: e.clientY - rect.top + borderW,
      x: e.clientX - rect.left - rect.width / 2 + borderWidth + marginLeft,
      y: e.clientY - rect.top - rect.height / 2 + borderWidth + marginLeft,
    }
