  ...
  for (let i = 0; i < len1; i++) {
    ...
    if (exist) {
      if (hasAttatch || isAttatchNext) {
        ...
        if (i === 1) up.push({ text: c1, rowSpan: 2, colSpan: 3, style: "tableHeader" })
        else if (i === 3 || i === len1 - 1) up.push({ text: c1, rowSpan: 2, colSpan: 2, style: "tableHeader" })
        else up.push({ text: c1, rowSpan: 2, style: "tableHeader" })
        down.push({ text: c2 })
      }
    }
  }
