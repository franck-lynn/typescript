...
const docDefinition: TDocumentDefinitions = {
  ...
  content: [
    ...
    // {
    //   canvas: [{ type: "line", x1: 0, y1: 8, x2: 514, y2: 8, color: "#eee" }],
    // },
    {
      text: " ",
    ...
    },
    {
      layout: {
        hLineWidth: (rowIndex: number, node: ContentTable) => {
          if (rowIndex === 0 || rowIndex === 2 || rowIndex === node.table.body.length) return 2
          if (rowIndex < node.table.body.length - 2) return 1
          return 0
        },
        vLineWidth: () => 0,
        hLineColor: () => "gray",
