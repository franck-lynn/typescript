...
const focused = ref(-1)
const { valueKey, data, searchKey, modelValue } = toRefs(props)

const results: ComputedRef<Record<string, string>[]> = computed((): Record<string, string>[] => {
  if (!searchTerm.value.length || selected.value) return []

  return data.value.filter((entry: Record<string, string>) => {
    return entry[searchKey.value]!.toLowerCase().includes(searchTerm.value.toLowerCase())
  })
})
