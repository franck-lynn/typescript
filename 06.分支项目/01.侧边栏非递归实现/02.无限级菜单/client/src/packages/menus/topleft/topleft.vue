...
import { preOrder, handleClickInNarrow } from "./topleft-helper"
  ...
  <ul v-for="node in treeData" :key="node.id" class="relative flex text-yellow-400 select-none">
    ...
    <li v-if="node.deep === 0 && node.href && !node.children" class="flex w-full h-10 items-center cursor-pointer">
      <router-link v-if="node.href" :to="node.href" class="cursor-pointerss">
        <span v-if="node.icon" :class="[`iconfont ${node.icon}`]" class="inline-flex w-10 h-10 items-center pl-2" />
        <span v-if="node.title" class="px-2">
          {{ node.title }}
        </span>
      </router-link>
    </li>
    ...
    <li
      ...
      class="flex w-48 h-10 items-center cursor-default bg-purple-500 level_0_has_children"
      @click="onToggleClick(node)"
    >
      <span v-if="node.icon" :class="[`iconfont ${node.icon}`]" class="flex w-10 h-10 items-center pl-2" />
      <span v-if="node.title" class="truncate px-2"> {{ node.title }} </span>
    </li>
    <!-- :style="[{marginLeft: `${(node.deep!-1) * 12}rem`, marginTop: `${node.marginTop}rem` }  ]" -->
    <!--  v-if="node.deep !== 0 && currentId === node.rootId" -->
    <li
      v-if="node.deep !== 0 && currentId === node.rootId"
      class="flex w-48 h-10 items-center absolute top-full px-4 bg-purple-900"
      :class="[node.href ? 'cursor-pointer' : 'cursor-default']"
      :style="[{marginLeft: `${(node.deep! -1  ) * 12  -12}rem`, marginTop: `${node.marginTop}rem` }  ]"
      @click="onToggleClick(node)"
    >
