...
import {
  ...
  updateTree,
  handleClick,
  updateRootIdToEmptyByTraversal,
  updateRootIdByTraversal,
...
const onToggleClick = (node: IMenu, isNarrow: boolean) => {
  if (isNarrow) {
    handleClick(node, currentId, treeData)
    // if (node.deep === 0) {
    //   node.rootId = node.id
    ...
    //     if (rootNode) {
    //       updateRootIdToEmptyByTraversal(rootNode)
    //     }
    //   }
    // }
  } else {
    ...
    <li v-if="node.deep === 0 && node.href && !node.children" class="flex w-full h-10 items-center cursor-pointer">
      <router-link v-if="node.href" :to="node.href" class="cursor-pointerss">
        <span
          ...
          class="inline-flex w-11 h-10 items-center p-2"
        />
        <span v-if="node.title" :class="[{ 'px-1': !node.icon, hidden: isNarrow }]"> {{ node.title }} </span>
      </router-link>
    </li>
