  ...
  <OnClickOutside class="flex" @trigger="close">
    <ul v-for="node in treeData" :key="node.id" class="relative flex text-yellow-400 select-none">
      ...
      <!-- <li
        ...
        class="flex h-10 items-center cursor-default level_0_has_child"
        :style="[{ width: `${_width_level_0}rem` }]"
        @click="onToggleClick(node)"
      > -->
      <li
        v-if="node.children && node.deep === 0"
        class="flex h-10 items-center cursor-default level_0_has_child"
        :style="[{ width: `${node.parentWidth}rem` }]"
        @click="onToggleClick(node)"
      >
      ...
      <!-- <li
        ...
        :style="[{width: `${_width_level_1n}rem`, 
          ! - _width_level_0}rem 0 级菜单的宽度的偏移量, 为了与 0 级菜单平齐, 减去这个量, 会左移对齐
          ! _width_level_1n 1 级菜单及 下级菜单的宽度, 每级右偏移上级菜单的宽度
           marginLeft: `${(node.deep! -1  ) * _width_level_1n  -_width_level_0}rem`, 
      ...
      <li
        ...
        :class="[node.href ? 'cursor-pointer' : 'cursor-default']"
        :style="[{width: `${_width_level_1n}rem`, 
           marginLeft: `${(node.deep! -1  ) * _width_level_1n  - node.parentWidth!}rem`, 
           marginTop: `${node.marginTop}rem` }  ]"
        @click="onToggleClick(node)"
      >
