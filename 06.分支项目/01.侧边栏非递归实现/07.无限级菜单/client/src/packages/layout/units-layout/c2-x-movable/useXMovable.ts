...
// ! 为了避免resizable 的干扰, 引入这个变量作为判断
// ! 当移动按下鼠标时, 就不能 resize
import { resizeFlag } from "./useResizable"

import { isNarrow, position } from "../../../commons"
...
type HorizontalMovableOptions = {
  ...
  initialValue?: MaybeRef<PositionX>

  breakPoint?: MaybeRef<number>
...
export const useXMovable = (
  ...
  const breakPoint = toValue(options.breakPoint) ?? BREAK_POINT

  // const isNarrow = ref<boolean>(false) // ! 改用 全局变量状态

  // 要返回的点的坐标, 用于保存移动的点 p1 的坐标的, 与下面 p1 不同的是,
  // p1 是开始移动前的位置, position 是移动后的位置

  // const position = ref<PositionX>(toValue(options.initialValue) ?? MIN_WIDTH)
  ...
  const start = (e: PointerEvent) => {
    ...
    // 获取移动块的左边距和宽度
    const { left, width } = toValue(target)!.getBoundingClientRect()
...
export type UseXMovableReturn = ReturnType<typeof useXMovable>
